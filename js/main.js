...
function checkStatus (color) {
    ...
    {
        $('#status').html(`<b>Шах і мат!</b> Oops, <b>${color}</b> програв.`);
    }
    else if (game.insufficient_material())
    {
        $('#status').html(`Це <b>нічия!</b> (Недостатній матеріал)`);
    }
    else if (game.in_threefold_repetition())
    {
        $('#status').html(`Це <b>нічия!</b> (Потрійне повторення)`);
    }
    else if (game.in_stalemate())
    {
        $('#status').html(`Це <b>нічия!</b> ( Патова ситуація)`);
    }
    else if (game.in_draw())
    {
        $('#status').html(`Це <b>нічия!</b> (Правило 50 ходів)`);
    }
    else if (game.in_check())
    {
        $('#status').html(`Oops, <b>${color}</b> має <b>шах!</b>`);
        return false;
    }
    else
    {
        $('#status').html(`Нема мату, шах і мату, або нічия.`)
        return false;
    }
    return true;
}
